...
@interface LMEditBaseView() <UIGestureRecognizerDelegate>
...
@property (nonatomic, strong) LMRotateGestureRecognizer * stickerGesture;
@property (nonatomic, strong) LMWidthGestureRecognizer * widthGesture;
@property (nonatomic, strong) LMScaleGestureRecognizer * scaleGesture;

@end


@implementation LMEditBaseView
...
- (UIImageView *)rightBottomControl
{
    if (!_rightBottomControl) {
        _rightBottomControl = [[UIImageView alloc] init];
        _rightBottomControl.contentMode = UIViewContentModeScaleAspectFit;
    }
...
- (LMRotateGestureRecognizer *)stickerGesture
{
    if (!_stickerGesture) {
        _stickerGesture = [[LMRotateGestureRecognizer alloc] initWithTarget:self action:@selector(gestureSingleHandAction:) anchorView:self.contentView];
    }

    return _stickerGesture;
}

- (LMWidthGestureRecognizer *)widthGesture {
    if (!_widthGesture) {
        _widthGesture = [[LMWidthGestureRecognizer alloc] initWithTarget:self action:@selector(widthGestureSingleHandAction:) anchorView:self.contentView];
    }
    return _widthGesture;
}

- (LMScaleGestureRecognizer *)scaleGesture {
    if (!_scaleGesture) {
        _scaleGesture = [[LMScaleGestureRecognizer alloc] initWithTarget:self action:@selector(scaleGestureSingleHandAction:) anchorView:self.contentView];
    }
...
- (void)setUpSubviewsFrames
{
    ...
    self.rightBottomControl.frame = CGRectMake(CGRectGetMaxX(self.contentView.frame) - self.controlSize.width / 2.0, CGRectGetMaxY(self.contentView.frame) - self.controlSize.height / 2.0, self.controlSize.width, self.controlSize.height);
...
- (void)setupGestures
{
    ...
    [self.leftBottomControl addGestureRecognizer:self.stickerGesture];
    [self.rightBottomControl addGestureRecognizer:self.widthGesture];
    [self.leftTopControl addGestureRecognizer:self.scaleGesture];
}
...
- (void)relocalControlView
{
    ...
    self.rightBottomControl.center = CGPointApplyAffineTransform(CGPointMake(maxX, maxY), self.contentView.transform);
...
- (void)gestureTap:(UITapGestureRecognizer *)sender
{
    ...
    } else if (sender.view == self.rightBottomControl) {
        [self deleteFromSuperView];
    }
}

- (void)gestureSingleHandAction:(LMRotateGestureRecognizer *)sender
{
    self.contentView.transform = CGAffineTransformRotate(self.contentView.transform, sender.rotation);
    [sender reset];
    [self relocalControlView];
...
- (void)widthGestureSingleHandAction:(LMWidthGestureRecognizer *)sender {
    self.contentView.bounds = CGRectMake(self.contentView.bounds.origin.x, self.contentView.bounds.origin.y, sender.width, self.contentView.bounds.size.height);
    [self relocalControlView];
    [self drawShapeLayerAtFrame: self.contentView.bounds];
}

- (void)scaleGestureSingleHandAction:(LMScaleGestureRecognizer *)sender {
    [self scaleLimit:sender.scale];
    self.contentView.transform = CGAffineTransformScale(self.contentView.transform, sender.scale, sender.scale);
    [sender reset];
    [self relocalControlView];
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
    ...
    if (self.isActivity) {
        if ([self.rightBottomControl pointInside:[self convertPoint:point toView:self.rightBottomControl] withEvent:event]) {
...
#pragma mark -Setter
- (void)setRightBottomImage:(UIImage *)rightBottomImage {
    _rightBottomImage = rightBottomImage;
    _rightBottomControl.image = rightBottomImage;
}
...
- (void)setIsActivity:(BOOL)isActivity
{
    ...
    _rightBottomControl.hidden = !isActivity;
